...
def prompt_find_harvesting_limit_from_conversation(
    ...
    with user():
        lm += (
            "In the following conversation, the participants discuss their production"
            " activities. Determine whether there was an explicit agreement on a concrete widget production limit. Look"
            " for direct mention or agreement on a numerical production limit that the group"
            " agreed to keep during this conversation."
        )
        lm += f"\n\nConversation:\n"
        lm += f"{conversation_to_string_with_dash(conversation)}\n"
        lm += "Please provide the specific production limit per person as agreed upon in the conversation, if no limit was agreed upon, please answer N/A. "
